...
export default function App() {
    ...
    const playlist = usePlaylist(
        ...
        // autoLoadSubtitles callback
        subs.autoLoadSubtitles
    );

    useKeyboardShortcuts(player, subs.handlePrevSentence, subs.handleNextSentence);

    // --- PLAYLIST PERSISTENCE (AUTO-SAVE) ---
    // Moved here to coordinate saving both tabs(playlist) and metadata(subs)
    const autoSaveTimerRef = useRef<any>(null);
    useEffect(() => {
        if (playlist.isPlaylistLoaded) {
            if (autoSaveTimerRef.current) clearTimeout(autoSaveTimerRef.current);
            autoSaveTimerRef.current = setTimeout(() => {
                // 1. Save Tab Structure
                ...
                saveFullPlaylistToDB(allFiles, subs.videoSubtitlesMap, subs.playbackProgressMap)
                    .catch(e => console.error("Auto-save playlist DB failed", e));
            }, 1000);
        }
    ...
    const loadVideoFromFile = useCallback(async (file: File) => {
        ...
        if ((file as any).isPlaceholder) {
            if (!isElectron || !filePath) { alert(`Due to browser security, please re-select or drag-and-drop "${file.name}" to play it.`); return; }
        }
        ...
        subs.setErrorMsg(null);
        subs.setCurrentSegmentIndex(-1);
        // We do NOT reset isConverting here, as that tracks background jobs in Playlist

        // Reset buffers
        subs.audioDataCacheRef.current = null;
        subs.lockStateRef.current = null;
        ...
        if (subs.videoSubtitlesMap[key]) subs.setSubtitles(subs.videoSubtitlesMap[key]);
        else subs.setSubtitles([]);

    }, [player, subs.videoSubtitlesMap, subs.playbackProgressMap, subs]);

    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        const file = event.target.files?.[0];
        if (!file) return;

        playlist.setTabs(prev => prev.map(t => {
            if (t.id === playlist.activeTabId) {
                if (t.files.some(f => f.name === file.name && f.size === file.size)) return t;
                return { ...t, files: [...t.files, file] };
            }
            return t;
        }));

        // showVideoList(true) - logic usually implies opening the list, but we can assume it's fine or add state if needed.
        // The original had setShowVideoList. Let's add that back to App local state.
        setShowVideoList(true);

        const activeFileList = playlist.getActiveFileList();
        await subs.autoLoadSubtitles([...activeFileList, file], file);
    ...
    return (
        <div className="flex h-screen bg-black text-gray-100 font-sans overflow-hidden relative">
            ...
            <SubtitlePanel
                ...
                onExport={(f) => {
                    if (subs.subtitles.length === 0) return;
                    const content = f === 'srt' ? segmentsToSRT(subs.subtitles) : segmentsToVTT(subs.subtitles);
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-gray-900 relative">
                ...
                <VideoPlaylist
                    show={showVideoList}
                    tabs={playlist.tabs}
                    activeTabId={playlist.activeTabId}
                    onSwitchTab={playlist.setActiveTabId}
                    onAddTab={playlist.handleAddTab}
                    onRemoveTab={playlist.handleRemoveTab}
                    onRenameTab={playlist.handleRenameTab}
                    onReorderTabs={playlist.handleReorderTabs}
                    videoList={playlist.getActiveFileList()}
                    currentVideoFile={videoFile}
                    videoStatuses={playlist.videoStatuses}
                    draggedVideoIndex={playlist.draggedVideoIndex}
                    processingVideoKey={subs.processingVideoKey}
                    onLoadVideo={loadVideoFromFile}
                    onBatchUpload={playlist.handleBatchUpload}
                    onDeleteVideo={playlist.handleDeleteVideo}
                    onClearList={playlist.handleClearList}
                    onConvert={playlist.handleQueueConversion}
                    onCancelConversion={(e, file) => { e.stopPropagation(); const key = `${file.name}-${file.size}`; playlist.handleCancelConversion(key); }}
                    onDragStart={(e, i) => playlist.setDraggedVideoIndex(i)}
                    onDragOver={(e) => { e.preventDefault(); }}
                    onDrop={(e, i) => { e.preventDefault(); if (playlist.draggedVideoIndex === null) return; playlist.setTabs(prev => prev.map(t => { if (t.id === playlist.activeTabId) { const newList = [...t.files]; const [moved] = newList.splice(playlist.draggedVideoIndex, 1); newList.splice(i, 0, moved); return { ...t, files: newList }; } return t; })); playlist.setDraggedVideoIndex(null); }}
                    onDragEnd={() => playlist.setDraggedVideoIndex(null)}
                />

                <div className="flex-1 flex flex-col overflow-hidden">
                    <div style={{ height: layout.videoHeight }} className="bg-black flex items-center justify-center relative flex-shrink-0">
                        {videoSrc ? (
                            <video ref={player.videoRef} src={videoSrc} className="w-full h-full object-contain" onClick={player.togglePlayPause} onLoadedMetadata={handleLoadedMetadata} onEnded={() => player.setIsPlaying(false)} onError={() => !subs.errorMsg && subs.setErrorMsg("Browser cannot decode audio.")} playsInline />
                        ) : (
                    ...
                    <div style={{ height: layout.subtitleHeight }} className="bg-gray-900 p-6 text-center flex flex-col items-center justify-center flex-shrink-0 overflow-y-auto">
                        {isProcessingCurrent && subs.subtitles.length === 0 ? (<div className="flex flex-col items-center justify-center gap-2 animate-pulse"><span className="text-blue-400 text-sm font-medium">Analyzing Audio...</span></div>) : subs.errorMsg ? (<div className="flex items-center gap-2 text-red-400 text-sm bg-red-900/20 px-4 py-2 rounded"><AlertCircle size={16} /><span>{subs.errorMsg}</span></div>) : null}
                    ...
                    <div className="flex-1 min-h-0 bg-gray-950 overflow-hidden flex flex-col">
                        <WordDefinitionPanel definition={vocab.selectedWord} onAddToVocab={vocab.addToVocab} isSaved={vocab.selectedWord ? vocab.vocabulary.some(v => v.word === vocab.selectedWord!.word) : false} isLoading={vocab.loadingWord} onWordSearch={(w) => vocab.handleWordClick(w, "Manual search")} />
                    </div>
                </div>
                ...
                <VideoControls
                    ...
                    onPrevSentence={subs.handlePrevSentence}
                    onNextSentence={subs.handleNextSentence}
                    onPrevFrame={() => player.stepFrame('prev')}
                    onNextFrame={() => player.stepFrame('next')}
                    hasSubtitles={subs.subtitles.length > 0}
                    currentTime={player.currentTime}
                ...
                <div style={{ width: layout.rightPanelWidth }} className="bg-gray-950 border-l border-gray-800 flex flex-col flex-shrink-0 hidden md:flex">
                    <div className="p-4 border-b border-gray-800 bg-gray-900/50 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-1">
                            <label className="p-2 text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg transition-colors cursor-pointer" title="Import JSON"><FileUp size={16} /><input type="file" accept=".json" onChange={vocab.handleImportVocab} className="hidden" /></label>
                            <button onClick={vocab.handleExportVocab} className="p-2 text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg transition-colors" title="Export JSON"><Download size={16} /></button>
                        </div>
                    </div>
                    <div className="flex-1 overflow-y-auto p-0">{vocab.vocabulary.length === 0 ? (<div className="p-8 text-center text-gray-500 text-sm opacity-60"><p>No words saved yet.</p></div>) : (vocab.vocabulary.map((item) => (<div key={item.id} onClick={() => { vocab.setSelectedWord(item); if (player.videoRef.current && player.isPlaying) { player.videoRef.current.pause(); player.setIsPlaying(false); } }} className="p-4 border-b border-gray-800 group hover:bg-gray-900 cursor-pointer transition-colors"><div className="flex justify-between items-start mb-1"><span className="font-bold text-white text-lg">{item.word}</span><button onClick={(e) => { e.stopPropagation(); vocab.setVocabulary(v => v.filter(i => i.id !== item.id)); }} className="text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1"><Trash2 size={16} /></button></div><p className="text-sm text-gray-400 line-clamp-2">{item.meaning}</p></div>)))}</div>
                </div>
            )}
        </div>
    );
