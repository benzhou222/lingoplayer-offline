...
export const getAudioData = async (videoFile: File, forOffline: boolean): Promise<Float32Array | string> => {
    try {
        ...
        // In Electron, reading large files into ArrayBuffer causes Renderer Crash (OOM).
        // We MUST check for Electron environment and delegate to Native FFmpeg immediately,
        // skipping the browser decode attempt entirely for Electron.
        // @ts-ignore
        if (window.electron && window.electron.isElectron && (videoFile as any).path) {
            console.log("[AudioData] Electron detected. Skipping browser decode to prevent OOM. Using Native FFmpeg.");
        ...
        // Attempt 1: Native Browser Decode (Fastest for Browser)
